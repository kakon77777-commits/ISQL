...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp = sub.add_parser("locality-index-build", help="Build a derived v0.9 locality index from standalone ISN7 frames")
    sp.add_argument("--frames-dir", required=True)
    sp.add_argument("--index", required=True)

    sp = sub.add_parser("locality-index-info", help="Inspect a derived v0.9 locality index")
    sp.add_argument("--index", required=True)

    sp = sub.add_parser("locality-select", help="Recall candidate bases then rerank by actual ISD8 bytes")
    sp.add_argument("--index", required=True)
    sp.add_argument("--frames-dir", required=True)
    sp.add_argument("--target", required=True)
    sp.add_argument("--out", required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "locality-index-build":
            frames_dir = Path(args.frames_dir).resolve()
            rows = []
            for path in sorted(frames_dir.rglob("*.isql7")):
                if path.is_file():
                    rows.append((path.relative_to(frames_dir).as_posix(), path.read_bytes()))
            index = build_locality_index(rows)
            save_locality_index(args.index, index)
            print(_json({
                "schema": "isql.locality-index-build/v0.9",
                "entry_count": len(index.entries),
                "index_hash": index.content_hash(),
                "index_file": str(Path(args.index)),
            }))
            return 0

        if args.command == "locality-index-info":
            index = load_locality_index(args.index)
            print(_json({
                "schema": "isql.locality-index-info/v0.9",
                "entry_count": len(index.entries),
                "index_hash": index.content_hash(),
                "resolutions": sorted({entry.resolution for entry in index.entries}),
                "registry_hashes": sorted({entry.registry_hash for entry in index.entries}),
            }))
            return 0

        if args.command == "locality-select":
            index = load_locality_index(args.index)
            frames_dir = Path(args.frames_dir).resolve()
            def _load_base(ref: str) -> bytes:
            ...
            target = Path(args.target).read_bytes()
            result = select_locality_base(target, index, _load_base, top_k=args.top_k)
            out_path = Path(args.out)
            out_path.parent.mkdir(parents=True, exist_ok=True)
            out_path.write_bytes(result.frame)
            payload = result.to_dict()
